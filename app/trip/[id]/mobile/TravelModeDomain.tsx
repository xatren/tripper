  ...
  return (
    <div style={{ paddingTop: 12, paddingBottom: 'calc(24px + env(safe-area-inset-bottom))' }}>
      <section className="glass-standard" style={{ position: 'sticky', top: 0, zIndex: 5, borderRadius: 20, padding: 16, backdropFilter: 'blur(20px)', background: 'rgba(12,18,32,.84)', border: `1px solid ${tokens.glassStandardBorder}` }}>
        <div style={{ fontSize: 11, fontWeight: 850, letterSpacing: '.08em', color: tokens.accentLight }}>TODAY · TRAVEL MODE</div>
        {nextItem ? <>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginTop: 8 }}>
            <div style={{ flex: 1 }}><div style={{ fontSize: 20, fontWeight: 850, color: '#fff' }}>{nextItem.title}</div><div style={{ marginTop: 4, color: tokens.textMuted, fontSize: 12.5 }}>{formatTime(nextItem.start_at)}{nextItem.address ? ` · ${nextItem.address}` : ''}</div></div>
            <span style={{ padding: '6px 9px', borderRadius: 999, color: tokens.textSecondary, background: tokens.surfaceSolid, fontSize: 10.5, fontWeight: 800 }}>{statusLabel[nextItem.status]}</span>
          </div>
          {syncByItem[nextItem.id] && <div role="status" style={{ marginTop: 8, color: syncByItem[nextItem.id] === 'queued' ? tokens.warning : tokens.danger, fontSize: 11.5, fontWeight: 750 }}>{syncByItem[nextItem.id] === 'queued' ? 'On device · waiting to sync' : 'Sync failed · retry from Sync center'}</div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {story.map((storyItem, index) => (
            <div key={storyItem.id} style={{ display: 'grid', gridTemplateColumns: '46px 18px minmax(0,1fr)', gap: 8, minHeight: 76 }}>
              <time style={{ paddingTop: 14, color: tokens.textMuted, fontSize: 10.5, fontWeight: 750 }}>{formatTime(storyItem.occurredAt)}</time>
              ...
              <article style={{ marginBottom: 8, padding: '12px 13px', borderRadius: 16, background: tokens.surfaceSolid, border: `1px solid ${tokens.glassSubtleBorder}` }}>
                <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.06em', color: tokens.textMuted, fontWeight: 800 }}>{storyItem.kind === 'plan' ? 'Plan' : storyItem.kind === 'event' ? 'Journey' : storyItem.entry.visibility === 'private' ? 'Private memory' : 'Journal'}</div>
                ...
                {storyItem.kind !== 'plan' && (storyItem.kind === 'event' ? storyItem.event.created_by : storyItem.entry.created_by) === currentUserId && <div style={{ display: 'flex', gap: 4, marginTop: 5 }}>
                  {storyItem.kind === 'journal' && <button type="button" onClick={() => { setEditingEntryId(storyItem.entry.id); setNote(storyItem.entry.note ?? ''); setPrivateMemory(storyItem.entry.visibility === 'private'); setComposer('note') }} style={{ minWidth: 44, minHeight: 44, border: 0, background: 'transparent', color: tokens.textSecondary, fontSize: 11.5, fontWeight: 750 }}>Edit</button>}
                  <button type="button" onClick={() => void hideStoryItem(storyItem.kind, storyItem.kind === 'event' ? storyItem.event.id : storyItem.entry.id)} style={{ minWidth: 44, minHeight: 44, border: 0, background: 'transparent', color: tokens.textSecondary, fontSize: 11.5, fontWeight: 750 }}>Hide</button>
                </div>}
              </article>
            </div>
          ))}
      ...
      <BottomSheet open={composer === 'note' || composer === 'photo'} onClose={() => { if (!saving) { setComposer(null); setEditingEntryId(null) } }} titleId="quick-memory-title" title={composer === 'photo' ? 'Add a photo' : editingEntryId ? 'Edit note' : 'Add a note'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button type="button" onClick={() => void saveMemory()} disabled={saving || (!note.trim() && !photo)} style={{ minHeight: 48, border: 0, borderRadius: 14, background: ACCENT_GRADIENT, color: '#1a0800', fontWeight: 850 }}>{saving ? 'Saving…' : typeof navigator === 'undefined' || navigator.onLine ? 'Add memory' : 'Save on device'}</button>
        </div>
      </BottomSheet>
      ...
      <BottomSheet open={composer === 'unplanned'} onClose={() => !saving && setComposer(null)} titleId="unplanned-stop-title" title={`Unplanned stop · ${unplannedStep}/2`}>
        {unplannedStep === 1 ? <div style={{ display: 'grid', gap: 12 }}><input autoFocus value={unplannedTitle} onChange={(event) => setUnplannedTitle(event.target.value)} placeholder="Where did you stop?" style={{ minHeight: 48, padding: 12, borderRadius: 14, border: `1px solid ${tokens.glassStandardBorder}`, background: tokens.surfaceSolid, color: '#fff', fontSize: 15 }} /><button type="button" disabled={!unplannedTitle.trim()} onClick={() => setUnplannedStep(2)} style={{ minHeight: 48, border: 0, borderRadius: 14, background: ACCENT_GRADIENT, color: '#1a0800', fontWeight: 850 }}>Continue</button></div> : <div style={{ display: 'grid', gap: 12 }}><input value={unplannedAddress} onChange={(event) => setUnplannedAddress(event.target.value)} placeholder="Address (optional)" style={{ minHeight: 48, padding: 12, borderRadius: 14, border: `1px solid ${tokens.glassStandardBorder}`, background: tokens.surfaceSolid, color: '#fff', fontSize: 14 }} /><div style={{ color: tokens.textMuted, fontSize: 11.5 }}>No location is tracked. Only the address you type is stored.</div><button type="button" disabled={saving} onClick={() => void saveUnplanned()} style={{ minHeight: 48, border: 0, borderRadius: 14, background: ACCENT_GRADIENT, color: '#1a0800', fontWeight: 850 }}>{saving ? 'Adding…' : 'Add stop'}</button></div>}
      </BottomSheet>
    </div>
  )
}
