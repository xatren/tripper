  ...
  return (
    <div style={{ paddingTop: 12, paddingBottom: 'calc(24px + env(safe-area-inset-bottom))' }}>
      ...
      <BottomSheet open={composer === 'unplanned'} onClose={() => !saving && setComposer(null)} titleId="unplanned-stop-title" title={`Unplanned stop · ${unplannedStep}/2`}>
        {unplannedStep === 1 ? <div style={{ display: 'grid', gap: 12 }}><input autoFocus value={unplannedTitle} onChange={(event) => setUnplannedTitle(event.target.value)} placeholder="Where did you stop?" style={{ minHeight: 48, padding: 12, borderRadius: 14, border: `1px solid ${tokens.glassStandardBorder}`, background: tokens.surfaceSolid, color: DUSK.textPrimary, fontSize: 15 }} /><button type="button" disabled={!unplannedTitle.trim()} onClick={() => setUnplannedStep(2)} style={{ minHeight: 48, border: 0, borderRadius: 14, background: ACCENT_GRADIENT, color: DUSK.onAmber, fontWeight: 850 }}>Continue</button></div> : <div style={{ display: 'grid', gap: 12 }}><input value={unplannedAddress} onChange={(event) => setUnplannedAddress(event.target.value)} placeholder="Address (optional)" style={{ minHeight: 48, padding: 12, borderRadius: 14, border: `1px solid ${tokens.glassStandardBorder}`, background: tokens.surfaceSolid, color: DUSK.textPrimary, fontSize: 14 }} /><div style={{ color: tokens.textMuted, fontSize: 11.5 }}>No location is tracked. Only the address you type is stored.</div><button type="button" disabled={saving} onClick={() => void saveUnplanned()} style={{ minHeight: 48, border: 0, borderRadius: 14, background: ACCENT_GRADIENT, color: DUSK.onAmber, fontWeight: 850 }}>{saving ? 'Adding…' : 'Add stop'}</button></div>}
      </BottomSheet>
    </div>
  )
}
