-- Stops split into two classes, distinguished only by `nights`:
--
--   nights >= 1  overnight stop — defines the trip's days (big map pin)
--   nights  = 0  day stop / waypoint — adds no day, rides along on the day of
--                the leg it sits on (small map pin)
--
-- Before this migration every stop was born with `nights = 1`, so a 22-stop
-- route produced a 22-night trip regardless of the dates the traveler picked.
-- The `check (nights >= 0)` constraint from 008_trip_persistence already allows
-- 0, so only the default changes here.
--
-- Existing rows are deliberately left alone: every stop stays at whatever it
-- has today, so this migration changes no visible trip length. Converting an
-- existing route to day stops is a user-confirmed action in the Plan tab.
alter table public.stops alter column nights set default 0;

-- The New Trip wizard's stops are destinations the traveler sleeps at, so the
-- atomic creation RPC keeps defaulting them to one night — the new column
-- default would otherwise turn every wizard trip into a zero-day trip. Callers
-- may now pass an explicit per-stop `nights`.
...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(stop) <> 'object'
       ...
       or (stop ? 'nights' and coalesce(stop->>'nights', '') !~ '^\d+$')
  ) then
    raise exception 'Every stop must have a name, valid coordinates, order, type, and night count';
  end if;
  ...
  insert into public.stops (trip_id, name, lat, lng, order_index, stop_type, nights, created_by)
  select
    v_trip_id,
    trim(s->>'name'),
